                ...
                move |data: &mut [f32], _: &cpal::OutputCallbackInfo| {
                    ...
                    if read < data.len() {
                        for (i, item) in data.iter_mut().enumerate().skip(read) {
                            ...
                            if channels == 2 {
                                if i % 2 == 0 {
                                    *item = last_left * underrun_fade;
                                } else {
                                    *item = last_right * underrun_fade;
                                }
                            } else {
                                *item = last_left * underrun_fade;
                            }
    ...
    fn decode_loop(
        ...
        let (audio_stream, mut audio_producer, target_sample_rate, target_channels) =
            if detected_format.is_some() {
                if let Some((stream, producer, rate, channels)) =
                    Self::setup_audio_output(Arc::clone(&is_playing), Arc::clone(&audio_buffer_len))
                {
