...
use wayland_overlay_media_viewer::{
    ...
    spawner::{
        CdBounce, CloseClickSettings, CustomShaderSource, Movement, ObjectCloseCondition,
        ObjectMessage, ObjectType, PopupImage, PopupInAnimation, PopupInteraction, PopupLayer,
        PopupOutAnimation, PopupSize, PopupVideo, TextPopup,
    },
};
...
fn main() {
    ...
    for screen in [0, 1] {
        for (hori, vert, size) in [
            (
                HorizontalAnchor::Left,
                VerticalAnchor::Top,
                PopupSize::ProportionalMin(400.0),
            ),
            (
                HorizontalAnchor::Left,
            ...
            (
                ...
                VerticalAnchor::Top,
                PopupSize::Custom(Vec2::new(300.0, 200.0)),
            ),
            (
                HorizontalAnchor::Right,
                VerticalAnchor::Bottom,
                PopupSize::ProportionalMax(400.0),
            ),
        ] {
            {
                _ = sender.send(ObjectMessage {
                    ...
                    position: PopupPosition::SceenSpace({
                        ScreenspacePosition {
                            screen: Some(screen),
                ...
                std::thread::spawn(move || {
                    ...
                    for entry in list {
                        _ = sender.send(ObjectMessage {
                            ...
                            movement: Movement::CdBounce(CdBounce {
                                speed: Vec2::new(1.0, 0.5),
                                angle_uncertanty: 1.1,
                                speed_uncertanty: 1.1,
                                speed_range: (0.1, 10.0),
                            }),
                            size: PopupSize::ProportionalMax(800.),
                        });
